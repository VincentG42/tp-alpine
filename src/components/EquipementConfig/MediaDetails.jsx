...
function MediaDetails({options, setShowMediaDetails}) {
    ...
    return (
        <div className='absolute z-10  backdrop-blur bg-slate-900 bg-opacity-30 h-full w-full flex justify-center items-center translate-y-36 md:translate-y-0'>
            <div id="driveModal" className='bg-slate-100 border border-slate-900 text-slate-900 w-5/6 py-16 flex flex-col gap-8 p-8 relative' >
                ...
                <div className='flex  flex-col gap-1'>
                    ...
                    {options.equipements.mediaAndNavigation.audio.map((assist) =>
                        <div key={assist.code}>
                            ...
                            <label htmlFor={assist.code} className='flex flex-col md:flex-row justify-between gap-2 px-4 py-2 text-sm md:text-base' onClick={() => handleAudioChange(assist.name, assist.price, assist.code)}>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <div>
                        ...
                        <label htmlFor={options.equipements.mediaAndNavigation.telemetrics.code} className='flex flex-col md:flex-row justify-between gap-2 px-4 py-2 text-sm md:text-base' onClick={() => handleTelemetrics(options.equipements.mediaAndNavigation.telemetrics.name, options.equipements.mediaAndNavigation.telemetrics.price)}>
